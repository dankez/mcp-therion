...
def add_to_studnica(topic: str, trick: str):
    ...
    # Použitie cache ak je k dispozícii, inak načítanie z disku
    if _studnica_cache is not None:
        data = _studnica_cache.copy()
    else:
        data = {}
    ...
    try:
        ...
        _studnica_cache = data  # Aktualizácia cache
...
def read_anonymized_th(rel_path: str):
    """Starý názov pre kompatibilitu s testami."""
    return read_therion_file(rel_path, anonymize=True)
...
def read_therion_file(rel_path: str, anonymize: bool = False):
    ...
    if not os.path.exists(full_path) and not rel_path.endswith('.thconfig'):
        ...
        if not os.path.exists(potential_path):
             return f"Chyba pri čítaní súboru: {rel_path} nebol nájdený."
...
def compile_therion(rel_path: str):
    ...
    if not os.path.exists(os.path.join(DATA_ROOT, rel_path)):
